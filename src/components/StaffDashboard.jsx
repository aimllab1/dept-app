...
import { Bell, BellDot, X, ImagePlus, ChevronRight } from 'lucide-react';
...
function StaffDashboard() {
  ...
  // Preview States
  const [staffImgPreview, setStaffImgPreview] = useState(null);
  ...
  const globalNotifications = useQuery(api.notifications.getGlobalNotifications) || [];
  ...
  const dashboardData = useMemo(() => {
    ...
    const stats = { present: 0, absent: 0, leave: 0, od: 0, holiday: 0, total: yearStudents.length };
    const absentDetails = [];
    yearAttendance.forEach(a => {
      if(stats[a.status] !== undefined) stats[a.status]++;
      if(a.status !== 'present' && a.status !== 'holiday' && a.status !== 'od') {
  ...
  // STABLE STATE SYNCS
  useEffect(() => {
    if (attendanceData && Object.keys(localAttendance).length === 0) {
      const map = {};
  ...
  // Reset local attendance when date or year changes to force a fresh pull
  useEffect(() => {
    setLocalAttendance({});
  ...
  const handlePostEvent = async (e) => { 
    e.preventDefault(); 
    const requesterId = loggedInUser._id === "hidden_admin" ? "hidden_admin" : loggedInUser._id.toString();
    await postEventMutation({ ...eventForm, requesterId, postedBy: loggedInUser.name }); 
    ...
    setEventForm({ title: '', description: '', type: 'Symposium', imageUrl: '', eventUrl: '' }); 
  ...
  const handleImageUpload = async (e, callback, previewCallback) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        img.onload = async () => {
          ...
          if (previewCallback) previewCallback(dataUrl);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-left">
      <aside className={`fixed inset-0 z-40 bg-white transform transition-transform md:relative md:translate-x-0 md:w-80 md:flex flex-col h-screen p-8 border-r border-gray-100 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
         <div className="mb-12">
            ...
            <div className="p-5 bg-blue-50 rounded-3xl border border-blue-100 flex justify-between items-center">
               ...
               <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 text-gray-400 hover:text-blue-600 transition-colors">
                  <Bell className="w-5 h-5" />
                  {globalNotifications.length > 0 && <span className="absolute top-0 right-0 w-3 h-3 bg-rose-500 border-2 border-white rounded-full"></span>}
               </button>
            </div>
         </div>
      ...
      <main className="flex-1 p-4 sm:p-10 lg:p-12 overflow-y-auto w-full">
         ...
         {showNotifications && (
           <div className="fixed inset-0 z-50 flex items-start justify-end p-6 pointer-events-none">
              <div className="w-80 bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden pointer-events-auto animate-fade-in">
                 <div className="p-6 border-b border-gray-50 flex justify-between items-center">
                    <h4 className="text-xs font-black uppercase text-gray-900">Recent Activity</h4>
                    <button onClick={() => setShowNotifications(false)}><X className="w-4 h-4 text-gray-400" /></button>
                 </div>
                 <div className="max-h-96 overflow-y-auto divide-y divide-gray-50">
                    {globalNotifications.length === 0 ? (
                      <div className="p-10 text-center text-gray-300 font-bold uppercase text-[9px]">No alerts</div>
                    ) : (
                      globalNotifications.map(n => (
                        <div key={n._id} onClick={() => handleClearNotification(n._id)} className="p-5 hover:bg-gray-50 cursor-pointer transition-all relative group">
                           <p className="text-[10px] font-black text-blue-600 uppercase mb-1">{n.title}</p>
                           <p className="text-xs font-medium text-gray-600">{n.message}</p>
                           <span className="absolute bottom-2 right-2 text-[8px] font-black text-blue-300 opacity-0 group-hover:opacity-100 transition-opacity">CLEAR</span>
                        </div>
                      ))
                    )}
                 </div>
         ...
         <div className="max-w-6xl mx-auto w-full text-left">
            ...
            {activeView === 'dashboard' && (
               <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                     <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 border-l-8 border-l-emerald-500 text-left"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Present</p><p className="text-4xl font-black text-gray-900">{dashboardData.stats.present}</p></div>
                     <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 border-l-8 border-l-rose-500 text-left"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Absent</p><p className="text-4xl font-black text-gray-900">{dashboardData.stats.absent}</p></div>
                     <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 border-l-8 border-l-orange-500 text-left"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Leave</p><p className="text-4xl font-black text-gray-900">{dashboardData.stats.leave}</p></div>
                     <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 border-l-8 border-l-blue-500 text-left"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">OD</p><p className="text-4xl font-black text-gray-900">{dashboardData.stats.od}</p></div>
                  </div>
                  <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden text-left">
                     ...
                     <div className="divide-y divide-gray-50 text-left">
                        {dashboardData.absentDetails.length === 0 ? <div className="p-16 text-center text-gray-300 font-bold uppercase text-[10px] tracking-widest">All Students Present</div> : 
                           dashboardData.absentDetails.map((a, i) => <div key={i} className="p-6 flex items-center justify-between px-10 text-left"><div className="text-left"><p className="font-black text-gray-900 uppercase tracking-tight text-left">{a.name}</p><p className="text-[8px] font-bold text-gray-400 uppercase text-left">{a.registrationNo}</p></div><span className={`text-[10px] font-black uppercase px-4 py-1.5 rounded-full ${a.status === 'absent' ? 'bg-rose-50 text-rose-600' : 'bg-orange-50 text-orange-600'}`}>{a.status}</span></div>)
                        }
                     </div>
                  </div>
               </div>
                     ...
                     <div className="divide-y divide-gray-50">
                        {filteredAttendanceStudents.map(s => (
                           <div key={s._id} onClick={() => {
                              const sequence = ['present', 'absent', 'leave', 'od', 'holiday'];
                              const current = localAttendance[s._id] || 'holiday';
                              ...
                              <div className="flex items-center space-x-6 text-left">
                                 <div className={`w-12 h-12 rounded-2xl border-4 border-white shadow-md overflow-hidden bg-gray-100`}>
                                    <img src={s.profileImage || `https://ui-avatars.com/api/?name=${s.name}`} className="w-full h-full object-cover" alt="" />
                                 </div>
                                 <div className="text-left">
                                    <p className="font-black text-gray-900 uppercase tracking-tight group-hover:text-blue-600">{s.name}</p>
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">
                                       {s.registrationNo} • 
                                       <span className={`ml-2 font-black ${
                                          localAttendance[s._id] === 'present' ? 'text-emerald-600' : 
                                          localAttendance[s._id] === 'absent' ? 'text-rose-600' : 
                                          localAttendance[s._id] === 'leave' ? 'text-orange-600' : 
                                          localAttendance[s._id] === 'od' ? 'text-blue-600' : 
                                          'text-gray-300'
                                       } uppercase`}>
                                          {localAttendance[s._id] === 'holiday' ? 'Normal (Holiday)' : (localAttendance[s._id] || 'Normal (Holiday)')}
                                       </span>
                                    </p>
                                 </div>
                              </div>
                              <div className={`w-5 h-5 rounded-full border-2 transition-all duration-300 flex items-center justify-center ${
                                 localAttendance[s._id] === 'present' ? 'bg-emerald-500 border-emerald-200 shadow-lg shadow-emerald-100' : 
                                 localAttendance[s._id] === 'absent' ? 'bg-rose-500 border-rose-200 shadow-lg shadow-rose-100' : 
                                 localAttendance[s._id] === 'leave' ? 'bg-orange-500 border-orange-200 shadow-lg shadow-orange-100' : 
                                 localAttendance[s._id] === 'od' ? 'bg-blue-500 border-blue-200 shadow-lg shadow-blue-100' : 
                                 localAttendance[s._id] === 'holiday' ? 'bg-slate-100 border-slate-200' : 
                                 'bg-white border-gray-100'
                              }`}>
                                 {localAttendance[s._id] === 'holiday' && <div className="w-1.5 h-1.5 bg-slate-300 rounded-full"></div>}
                              </div>
                           </div>
                        ))}
                     </div>
                     <div className="p-8 bg-gray-50 flex justify-end text-left"><button onClick={handleSubmitAttendance} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-blue-100 hover:bg-blue-700 active:scale-95 transition-all text-left">Sync to Cloud</button></div>
                  </div>
               </div>
            )}
                     ...
                     <div className="bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden animate-fade-in text-left">
                        ...
                        <div className="p-8 space-y-4 text-left">
                           {subjects.filter(s => s.semester === resultsSem).map(sub => (
                              <div key={sub._id} className="flex items-center justify-between p-6 bg-gray-50 rounded-3xl text-left">
               ...
               <div className="space-y-8">
                  ...
                  {marksBatchId && bulkMarkSubjectId && (
                     <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden text-left animate-fade-in">
                        ...
                        <div className="divide-y divide-gray-50 max-h-[500px] overflow-y-auto text-left">
                           {students
                              .filter(s => (batches.find(b => b._id === marksBatchId)?.name === s.currentBatch))
                              .sort((a, b) => (a.registrationNo || "").localeCompare(b.registrationNo || ""))
                              .map(s => (
                              <div key={s._id} className="p-6 flex items-center justify-between hover:bg-blue-50/30 text-left"><div className="text-left"><p className="font-bold text-gray-900 uppercase leading-none">{s.name}</p><p className="text-[10px] font-bold text-gray-400 mt-1">{s.registrationNo}</p></div><input className={`w-24 text-center p-3 bg-gray-50 border border-gray-100 rounded-xl font-black outline-none ${parseInt(studentMarks[s._id]) < 50 ? 'text-rose-600' : 'text-blue-600'}`} placeholder="0" value={studentMarks[s._id] || ''} onChange={e => setStudentMarks(prev => ({...prev, [s._id]: e.target.value}))} /></div>
                           ))}
                        </div>
                        <div className="p-8 bg-gray-50 flex justify-end text-left"><button onClick={async () => { 
                           const marksData = Object.entries(studentMarks).map(([sid, sc]) => ({ studentId: sid, score: sc })); 
                           const requesterId = loggedInUser._id === "hidden_admin" ? "hidden_admin" : loggedInUser._id.toString();
                           await postBulkMarksMutation({ subjectId: bulkMarkSubjectId, testType: bulkTestType, marks: marksData }); 
                           
                           // Individual Notifications for marks
                           for (const entry of marksData) {
                             await createNotification({
                               userId: entry.studentId,
                               title: "New Mark Entered",
                               message: `Your score for ${bulkTestType} in ${subjects.find(s => s._id === bulkMarkSubjectId)?.name} is ${entry.score}.`,
                               type: "mark"
                             });
                           }
                           
                           alert('Applied!'); 
                        }} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black uppercase shadow-lg shadow-blue-100 active:scale-95 transition-all text-left">Apply Scores</button></div>
                     </div>
                  )}
               ...
               <div className="space-y-8 text-left">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
                     <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 text-left">
                        <h3 className="text-xl font-black mb-8 uppercase text-left">Publish</h3>
                        <form onSubmit={handlePostEvent} className="space-y-6 text-left">
                           ...
                           <div className="grid grid-cols-2 gap-4 text-left">
                              <select className="input-field text-left" value={eventForm.type} onChange={e => setEventForm({...eventForm, type: e.target.value})}>
                              ...
                              <div className="relative group cursor-pointer text-left">
                                 <input type="file" accept="image/*" className="hidden" id="staff-img-ev" onChange={e => handleImageUpload(e, (res) => setEventForm({...eventForm, imageUrl: res}), setEventImgPreview)} />
                                 <label htmlFor="staff-img-ev" className="w-full aspect-[4/3] flex items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl text-[10px] font-black text-gray-400 uppercase overflow-hidden">
                                    {eventImgPreview ? <img src={eventImgPreview} className="w-full h-full object-cover text-left" alt="" /> : <ImagePlus className="w-6 h-6" />}
                                 </label>
                              </div>
                           </div>
                     ...
                     <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden text-left"><div className="p-8 border-b bg-gray-50 font-black uppercase text-xs text-left">History</div><div className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto text-left">{announcements.map(a => (
                        <div key={a._id} className="p-6 text-left">
            ...
            {activeView === 'directory' && (
               <div className="space-y-10 text-left">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 text-left">
                     ...
                     {isAdmin && <button onClick={() => setActiveView('add-student')} className="bg-blue-600 text-white px-8 py-3 rounded-full font-black text-[10px] uppercase shadow-lg transition-all active:scale-95 text-left">Enroll New</button>}
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left">
                     <div className="lg:col-span-4 space-y-8 text-left">
                        <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 text-left">
                           <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-6 text-left">Active Batches</h3>
                           ...
                           {isAdmin && <button onClick={() => setIsAddingBatch(!isAddingBatch)} className="w-full mt-6 py-3 border-2 border-dashed border-gray-200 rounded-2xl text-[10px] font-black text-gray-400 uppercase tracking-widest hover:border-blue-200 hover:text-blue-600 transition-all text-left">{isAddingBatch ? 'Cancel' : '+ New Batch'}</button>}
                        </div>
                        {isAdmin && isAddingBatch && (
                           <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 text-left animate-fade-in text-left">
                              <form onSubmit={async (e) => { e.preventDefault(); await createBatchMutation({...batchForm, requesterId: loggedInUser._id.toString()}); alert('Done!'); setIsAddingBatch(false); }} className="space-y-4 text-left"><input className="input-field text-left" placeholder="Name" value={batchForm.name} onChange={e => setBatchForm({...batchForm, name: e.target.value})} required /><div className="grid grid-cols-2 gap-4 text-left"><input type="number" className="input-field text-left" placeholder="Start" value={batchForm.startYear} onChange={e => setBatchForm({...batchForm, startYear: parseInt(e.target.value)})} required /><input type="number" className="input-field text-left" placeholder="End" value={batchForm.endYear} onChange={e => setBatchForm({...batchForm, endYear: parseInt(e.target.value)})} required /></div><button type="submit" className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl text-left">Confirm</button></form>
                           </div>
                        )}
                     </div>
                     <div className="lg:col-span-8 text-left">
                        {!directoryBatchId ? (
                           <div className="bg-white rounded-[3rem] p-20 shadow-xl border border-gray-100 flex flex-col items-center justify-center text-center text-left"><div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center text-3xl mb-6 text-left">🎓</div><h3 className="text-lg font-black text-gray-900 uppercase text-left">Choose a batch</h3></div>
                        ) : (
                           <div className="bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden animate-fade-in text-left">
                              <div className="p-8 border-b bg-gray-50 flex justify-between items-center text-left"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-left">List: {batches.find(b => b._id === directoryBatchId)?.name}</span></div>
                              <div className="divide-y divide-gray-50 text-left">
                                 {students
                                    .filter(s => s.currentBatch === batches.find(b => b._id === directoryBatchId)?.name)
                                    .sort((a, b) => (a.registrationNo || "").localeCompare(b.registrationNo || ""))
                                    .map(s => (
                                    <div key={s._id} className="p-6 flex items-center justify-between hover:bg-gray-50 transition-all text-left">
                                       <div className="flex items-center space-x-4 text-left">
                                          <div className="w-10 h-10 rounded-xl bg-gray-100 overflow-hidden border border-gray-200 text-left"><img src={s.profileImage || `https://ui-avatars.com/api/?name=${s.name}`} className="w-full h-full object-cover text-left" alt="" /></div>
                                          <div className="text-left"><p className="font-bold text-gray-900 uppercase tracking-tight text-left">{s.name}</p><p className="text-[10px] font-black text-gray-400 uppercase mt-1 text-left">{s.registrationNo}</p></div>
                                       </div>
                                       {isAdmin ? (
                                         <div className="flex gap-2">
                                           <button onClick={() => setEditingStudent(s)} className="text-[10px] font-black text-blue-600 border border-blue-100 px-4 py-2 rounded-full hover:bg-blue-600 hover:text-white transition-all text-left">Edit</button>
                                           <button onClick={async () => { if(window.confirm(`Permanently remove ${s.name}?`)) { await removeUserMutation({requesterId: loggedInUser._id.toString(), id: s._id}); alert('Student Removed'); }}} className="text-[10px] font-black text-rose-600 border border-rose-100 px-4 py-2 rounded-full hover:bg-rose-600 hover:text-white transition-all text-left">Del</button>
                                         </div>
                                       ) : <button onClick={() => navigate('/student-dashboard', { state: { user: s } })} className="text-[10px] font-black text-indigo-600 border border-indigo-100 px-4 py-2 rounded-full text-left">View</button>}
                                    </div>
                                 ))}
                              </div>
                           </div>
            ...
            {activeView === 'subjects' && isAdmin && (
               <div className="space-y-8 text-left">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 text-left">
                    <h2 className="text-3xl font-black text-gray-900 uppercase text-left">Curriculum</h2>
                    <button onClick={() => setActiveView('add-subject')} className="bg-blue-600 text-white px-6 py-3 rounded-full font-black text-[10px] uppercase shadow-lg text-left">New Subject</button>
                  </div>
                  <div className="flex flex-wrap gap-2 bg-gray-200/50 p-1.5 rounded-2xl text-left">
                  ...
                  <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden text-left">
                     <div className="divide-y divide-gray-100 text-left">
                        {subjects.filter(s => s.semester === selectedSemTab).map(sub => (
                           <div key={sub._id} className="p-8 flex items-center justify-between hover:bg-gray-50 transition-all text-left">
                              <div className="text-left"><p className="font-black text-gray-900 uppercase text-lg leading-none">{sub.name}</p><p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-2">{sub.code} • {sub.staffName}</p></div>
                              <div className="flex space-x-4">
                                 <button onClick={() => setEditingSubject(sub)} className="text-[10px] font-black text-gray-400 hover:text-blue-600 uppercase tracking-widest text-left">Reassign</button>
                                 <button onClick={async () => { if(window.confirm(`Delete ${sub.name}?`)) { await removeSubjectMutation({requesterId: loggedInUser._id.toString(), id: sub._id}); alert('Deleted'); }}} className="text-[10px] font-black text-rose-400 hover:text-rose-600 uppercase tracking-widest text-left">Delete</button>
                              </div>
                           </div>
                        ))}
                     </div>
            ...
            {activeView === 'staff' && isAdmin && (
               <div className="space-y-8 text-left">
                  <h2 className="text-3xl font-black text-gray-900 uppercase text-left">Faculty Registry</h2>
                  <div className="grid grid-cols-1 gap-8 text-left">
                     <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden text-left">
                        <div className="divide-y divide-gray-100 text-left">
                           {allStaff.map(s => (
                              <div key={s._id} className="p-6 flex items-center justify-between text-left">
                                 <div className="flex items-center space-x-4 text-left">
                                    <div className="w-10 h-10 rounded-xl bg-gray-100 overflow-hidden text-left"><img src={s.profileImage || `https://ui-avatars.com/api/?name=${s.name}`} className="w-full h-full object-cover text-left" alt="" /></div>
                                    <div className="text-left"><p className="font-bold text-gray-900 uppercase text-left">{s.name}</p><p className="text-[10px] font-black text-blue-600 uppercase text-left">{s.role.toUpperCase()}</p></div>
                                 </div>
                                 <div className="flex space-x-2 text-left">
                                    <button onClick={() => setEditingStaff(s)} className="text-blue-600 text-[10px] font-black uppercase border border-blue-100 px-4 py-2 rounded-full hover:bg-blue-50 transition-all text-left">Edit</button>
                                    {s.role === 'staff' && <button onClick={async () => { if(window.confirm('Remove?')) { await removeUserMutation({requesterId: loggedInUser._id.toString(), id: s._id}); alert('Removed'); }}} className="text-rose-600 text-[10px] font-black uppercase border border-rose-100 px-4 py-2 rounded-full hover:bg-rose-50 transition-all text-left">Del</button>}
                                 </div>
                              </div>
                           ))}
                        </div>
                     ...
                     {isAddingStaff && (
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 text-left animate-fade-in max-w-2xl mx-auto w-full">
                           ...
                           <div className="flex justify-center mb-8 text-left">
                              <div className="relative group cursor-pointer text-left">
                                 <input type="file" accept="image/*" className="hidden" id="staff-img-sc" onChange={e => handleImageUpload(e, (res) => setStaffForm({...staffForm, profileImage: res}), setStaffImgPreview)} />
                                 <label htmlFor="staff-img-sc" className="block w-32 h-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden bg-gray-50 text-left">
                                    {staffImgPreview ? <img src={staffImgPreview} className="w-full h-full object-cover text-left" alt="" /> : <div className="w-full h-full flex items-center justify-center text-gray-300 text-left">Photo</div>}
                                 </label>
                              </div>
                           </div>
                           <form onSubmit={async (e) => { 
                              e.preventDefault(); 
                              try {
                                 await addStaffMutation({...staffForm, requesterId: loggedInUser._id.toString()}); 
                                 alert('Done!'); 
                                 setStaffForm({name:'', password:'', role:'', profileImage:'', address: ''}); 
                                 setStaffImgPreview(null);
            ...
            {activeView === 'add-student' && isAdmin && (
               <div className="space-y-8 text-left">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight text-left">New Student</h2>
                  </div>
                  <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-gray-100 max-w-3xl mx-auto text-left">
                     <form onSubmit={async (e) => { 
                        e.preventDefault(); 
                        try {
                        ...
                        <div className="sm:col-span-2 flex justify-center mb-4 text-left"><div className="relative group cursor-pointer text-left text-left"><input type="file" accept="image/*" className="hidden" id="enroll-img-sc" onChange={e => handleImageUpload(e, (res) => setStudentForm({...studentForm, profileImage: res}), setStudentImgPreview)} /><label htmlFor="enroll-img-sc" className="block w-24 h-24 rounded-3xl border-2 border-dashed border-gray-200 overflow-hidden bg-gray-50 text-left text-left text-left">{studentImgPreview ? <img src={studentImgPreview} className="w-full h-full object-cover text-left" alt="" /> : <div className="w-full h-full flex items-center justify-center text-gray-300 text-left">Photo</div>}</label></div></div>
                        <input className="input-field sm:col-span-2 text-left" placeholder="Name" value={studentForm.name} onChange={e => setStudentForm({...studentForm, name: e.target.value})} required />
                        <input className="input-field text-left" placeholder="Reg No" value={studentForm.registrationNo} onChange={e => setStudentForm({...studentForm, registrationNo: e.target.value})} required />
                        <div className="space-y-2">
                        ...
                        <input className="input-field text-left" placeholder="Parent" value={studentForm.parentMobileNo} onChange={e => setStudentForm({...studentForm, parentMobileNo: e.target.value})} required />
                        <select className="input-field sm:col-span-2 text-left" value={studentForm.batchId} onChange={e => setStudentForm({...studentForm, batchId: e.target.value})} required><option value="">Batch</option>{batches.map(b => <option key={b._id} value={b._id}>{b.name}</option>)}</select>
                        ...
                        <button type="submit" className="sm:col-span-2 w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase shadow-xl text-left">Confirm Enrollment</button>
                     </form>
                  </div>
               </div>
            )}
            ...
            {activeView === 'security' && (
               <div className="space-y-8 text-left">
                  <h2 className="text-3xl font-black text-gray-900 uppercase text-left">Account Settings</h2>
                  <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 max-w-md mx-auto text-left"><h3 className="text-xl font-black mb-8 uppercase text-center text-left">Update Credentials</h3><form onSubmit={async (e) => { e.preventDefault(); if (loggedInUser._id === "hidden_admin") return alert("Locked."); await changePasswordMutation({ requesterId: loggedInUser._id.toString(), id: loggedInUser._id, newPassword }); alert("Updated!"); setNewPassword(''); }} className="space-y-6 text-left"><input type="password" placeholder="New Password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="input-field text-center text-left" required /><button type="submit" className="w-full py-5 bg-gray-900 text-white rounded-2xl font-black uppercase text-left">Apply</button></form></div>
               </div>
            )}
         </div>
      </main>
      ...
      {editingStudent && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-10 text-left">
           <div className="bg-white rounded-[3rem] shadow-2xl p-8 sm:p-12 w-full max-w-5xl h-full overflow-y-auto text-left">
              <div className="flex justify-between items-center mb-10 text-left"><h3 className="text-2xl font-black text-gray-900 uppercase text-left">Profile Records</h3><button onClick={() => setEditingStudent(null)} className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center font-bold text-gray-500 hover:bg-gray-200 text-left">✕</button></div>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 text-left">
                 <div className="lg:col-span-5 space-y-8 text-left">
                    <div className="flex justify-center text-left"><div className="relative group cursor-pointer text-left text-left"><input type="file" accept="image/*" className="hidden" id="edit-stu-sc" onChange={e => handleImageUpload(e, (res) => setEditingStudent({...editingStudent, profileImage: res}))} /><label htmlFor="edit-stu-sc" className="block w-32 h-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden bg-gray-50 text-left text-left text-left text-left text-left"><img src={editingStudent.profileImage || `https://ui-avatars.com/api/?name=${editingStudent.name}`} className="w-full h-full object-cover text-left" alt="" /></label></div></div>
                    <form onSubmit={async (e) => { 
                       e.preventDefault(); 
                       try {
                          const { _id, _creationTime, currentBatch, currentSemester, enrollmentId, role, ...data } = editingStudent; 
                          await updateStudentMutation({ requesterId: loggedInUser._id.toString(), id: _id, ...data }); 
                          alert('Updated!'); 
                       ...
                       <input className="input-field" value={editingStudent.parentMobileNo || ''} onChange={e => setEditingStudent({...editingStudent, parentMobileNo: e.target.value})} placeholder="Parent Mobile" />
                       <textarea className="input-field" placeholder="Residential Address" value={editingStudent.address || ''} onChange={e => setEditingStudent({...editingStudent, address: e.target.value})} />
                       <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-left">Save Changes</button>
                    </form>
                 </div>
                 <div className="lg:col-span-7 space-y-8 text-left">
                   <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] text-left">Attendance Timeline</h4>
                   <div className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100 text-left">
                     <StudentAttendanceHistoryEditor studentId={editingStudent._id} onMark={markAttendanceMutation} />
                   </div>
                 </div>
              </div>
           </div>
      ...
      {editingStaff && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-6 text-left">
           <div className="bg-white rounded-[3rem] shadow-2xl p-12 max-w-xl w-full text-left">
              <h3 className="text-2xl font-black text-gray-900 mb-8 uppercase text-left">Faculty Profile</h3>
              <div className="flex justify-center mb-8 text-left">
                 <div className="relative group cursor-pointer text-left text-left">
                    ...
                    <label htmlFor="edit-stf-sc" className="block w-32 h-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden bg-gray-50 text-left" alt="">
                       <img src={editingStaff.profileImage || `https://ui-avatars.com/api/?name=${editingStaff.name}`} className="w-full h-full object-cover text-left" alt="" />
                    </label>
                 </div>
              </div>
              <form onSubmit={async (e) => { e.preventDefault(); const { _id, _creationTime, role, ...data } = editingStaff; await updateStaffMutation({ requesterId: loggedInUser._id.toString(), id: _id, ...data }); alert('Staff Updated!'); setEditingStaff(null); }} className="space-y-6 text-left">
                 <input className="input-field text-left" value={editingStaff.name} onChange={e => setEditingStaff({...editingStaff, name: e.target.value})} /><input type="password" name="new_f_p_sc" className="input-field text-left" placeholder="New Password" value={editingStaff.password || ''} onChange={e => setEditingStaff({...editingStaff, password: e.target.value})} /><div className="flex gap-4 text-left"><button type="submit" className="flex-1 py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-left">Save Changes</button><button type="button" onClick={() => setEditingStaff(null)} className="flex-1 py-5 bg-gray-100 text-gray-900 rounded-2xl font-black uppercase text-left">Cancel</button></div>
              </form>
           </div>
        </div>
      )}
      ...
      {editingSubject && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-6 text-left">
           <div className="bg-white rounded-[3rem] shadow-2xl p-12 max-w-xl w-full text-left">
              <h3 className="text-2xl font-black text-gray-900 mb-8 uppercase text-left">Update Course</h3>
              <form onSubmit={async (e) => { e.preventDefault(); await updateSubjectMutation({requesterId: loggedInUser._id.toString(), id: editingSubject._id, name: editingSubject.name, code: editingSubject.code, semester: editingSubject.semester, staffId: editingSubject.staffId}); setEditingSubject(null); alert('Updated!'); }} className="space-y-6 text-left">
                <div className="text-left"><label className="text-[10px] font-black text-gray-400 uppercase text-left">Staff Handler</label><select className="input-field text-left" value={editingSubject.staffId || ''} onChange={e => setEditingSubject({...editingSubject, staffId: e.target.value})}><option value="">Unassigned</option>{allStaff.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}</select></div><button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase shadow-xl text-left">Sync Staff</button>
              </form>
           </div>
        </div>
      )}
      ...
      <style>{`
        .input-field { width: 100%; padding: 1.125rem 1.5rem; background-color: #f9fafb; border: 1px solid #f3f4f6; border-radius: 1.25rem; font-weight: 700; outline: none; transition: all 0.2s; font-size: 0.9rem; }
...
const StudentAttendanceHistoryEditor = ({ studentId, onMark }) => {
  ...
  const historyMap = useMemo(() => {
    const map = {};
    if (history) history.forEach((h) => { map[h.date] = h.status; });
    return map;
  }, [history]);
  const [localHistoryMap, setLocalHistoryMap] = useState({});

  useEffect(() => {
    setLocalHistoryMap(historyMap);
  }, [historyMap]);

  const cycle = async (day) => {
    const dateStr = `${viewMonth.getFullYear()}-${String(viewMonth.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    const sequence = ['present', 'absent', 'leave', 'od', 'holiday'];
    const current = localHistoryMap[dateStr] || 'holiday';

    const currentIndex = sequence.indexOf(current);
    const next = sequence[(currentIndex + 1) % sequence.length];

    setLocalHistoryMap((prev) => ({ ...prev, [dateStr]: next }));
    try {
      await onMark({ studentId, date: dateStr, status: next });
    } catch (err) {
      setLocalHistoryMap((prev) => ({ ...prev, [dateStr]: current }));
      console.error(err);
      const errorMessage = err?.data
        ? JSON.stringify(err.data)
        : (err?.message || "Unknown error");
      alert(`Failed to update attendance for ${dateStr}: ${errorMessage}`);
    }
  };

  return (<div className="w-full text-left">
    <div className="flex justify-between items-center mb-8">
      <button type="button" onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() - 1))} className="p-3 bg-white rounded-xl shadow-sm hover:bg-gray-100 transition-all">&larr;</button>
      <span className="font-black uppercase tracking-[0.2em] text-gray-900">{viewMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
      <button type="button" onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() + 1))} className="p-3 bg-white rounded-xl shadow-sm hover:bg-gray-100 transition-all">&rarr;</button>
    </div>
    <div className="grid grid-cols-7 gap-3 text-center">
      {['S','M','T','W','T','F','S'].map((d,i) => <div key={i} className="text-[10px] font-black text-gray-300 uppercase">{d}</div>)}
      ...
      {Array.from({length: new Date(viewMonth.getFullYear(), viewMonth.getMonth()+1, 0).getDate()}).map((_,i) => {
        const day = i+1;
        const dateStr = `${viewMonth.getFullYear()}-${String(viewMonth.getMonth()+1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
        const st = localHistoryMap[dateStr];
        return (
          <button
            type="button"
            key={day}
            onClick={() => cycle(day)}
            className={`h-11 rounded-xl flex items-center justify-center text-xs font-black transition-all border-2 ${
              st === 'present' ? 'bg-emerald-500 border-emerald-200 text-white shadow-lg shadow-emerald-100' :
              st === 'absent' ? 'bg-rose-500 border-rose-200 text-white shadow-lg shadow-rose-100' :
              st === 'leave' ? 'bg-orange-500 border-orange-200 text-white shadow-lg shadow-orange-100' :
              st === 'od' ? 'bg-blue-500 border-blue-200 text-white shadow-lg shadow-blue-100' :
              st === 'holiday' ? 'bg-slate-100 border-slate-200 text-slate-400' :
    ...
    <div className="mt-8 grid grid-cols-3 gap-2">
       <div className="text-[8px] font-black uppercase text-gray-400 flex items-center"><div className="w-2 h-2 rounded-full bg-emerald-500 mr-2"></div>Present</div>
       <div className="text-[8px] font-black uppercase text-gray-400 flex items-center"><div className="w-2 h-2 rounded-full bg-rose-500 mr-2"></div>Absent</div>
       <div className="text-[8px] font-black uppercase text-gray-400 flex items-center"><div className="w-2 h-2 rounded-full bg-orange-500 mr-2"></div>Leave</div>
       <div className="text-[8px] font-black uppercase text-gray-400 flex items-center"><div className="w-2 h-2 rounded-full bg-blue-500 mr-2"></div>OD</div>
       <div className="text-[8px] font-black uppercase text-gray-400 flex items-center"><div className="w-2 h-2 rounded-full border border-slate-300 bg-slate-100 mr-2"></div>Normal (Holiday)</div>
    </div>
  </div>);
};
